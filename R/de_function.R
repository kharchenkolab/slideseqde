...
#' Context-dependent DE approximation using a BVLS model
#'
#' @param profiles either a matrix of profiles (columns - cell types, rows - genes)
#'                or a list of such matrices, each corresponding to a replicate
#' @param target1 name of the cell type in context 1 (e.g. Fibroblast_Tumor)
...
#' @return
#' @export
getDifferentialExpressionBVLS <- function(profiles, target1, target2,
                                      de.method='edgeR.qlf',
                                      default.bcv=0.1 ,
                                      gene.scale.function=sqrt ,
                                      library.size.scale=1e6,
  ...
  for(p in profiles){
    if(!(target1 %in% colnames(p)) | !(target2 %in% colnames(p))){
      print(paste(target1,'or',target2,' NOT present in matrix'))
      return(NULL)
    }
  }
  ...
  ml <- lapply(profiles,function(p) {
    ...
    # take common genes
    p <- t(t(p)/colSums(p)*library.size.scale)*gene.scaling
    cnames = p[,colnames(p)!=target1] %>% colnames
    index = grep(target2,cnames)
    upper.limit = ifelse(grepl('Non_Tumor',cnames),0,Inf)
    upper.limit[[index]] = Inf
    lower.limit = ifelse(grepl('Non_Tumor',cnames),-10,0)
    lower.limit[[index]] = 0
    # upper.limit = ifelse(grepl('Non_Tumor',cnames),0,1)
    # upper.limit[[index]] = 1
    # lower.limit = ifelse(grepl('Non_Tumor',cnames),-1,0)
    # lower.limit[[index]] = 1
    m = bvls(
